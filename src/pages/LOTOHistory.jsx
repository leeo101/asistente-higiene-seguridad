...
import { ArrowLeft, Search, Lock, AlertTriangle, CheckCircle2, Key } from 'lucide-react';
...
export default function LOTOHistory() {
    ...
    const filteredProcedures = procedures.filter(proc => {
        const matchesSearch = proc.equipmentName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                            proc.area?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    return (
        <div className="container" style={{ paddingBottom: '6rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                ...
                <button onClick={() => navigate('/loto')} className="btn-primary" style={{ margin: 0, padding: '0.75rem 1.25rem' }}>
                    Nuevo Procedimiento
                </button>
            </div>
            ...
            {filteredProcedures.length === 0 ? (
                <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
                    ...
                    <button onClick={() => navigate('/loto')} className="btn-primary" style={{ marginTop: '1rem' }}>Crear Primer Procedimiento</button>
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {filteredProcedures.map(proc => (<ProcedureCard key={proc.id} procedure={proc} onClick={() => navigate(`/loto/${proc.id}`)} />))}
                </div>
            )}
        </div>
    );
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>{procedure.area || 'Sin área'} • {procedure.energyTypes?.length || 0} energías</p>
                    </div>
                </div>
