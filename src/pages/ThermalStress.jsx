...
import {
    ArrowLeft, Save, ThermometerSun, AlertTriangle,
    Calculator, Info, RefreshCw, Printer, Search, Settings2
} from 'lucide-react';
...
export default function ThermalStress() {
    ...
    const handleSave = () => {
        ...
        const history = JSON.parse(localStorage.getItem('thermal_history') || '[]');
        history.unshift(report);
        localStorage.setItem('thermal_history', JSON.stringify(history));
        syncCollection('thermal_history', history);

        toast.success('Medición guardada en el historial.');
            ...
            <div className="grid-2-cols" style={{ gap: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div className="card shadow-xl" style={{ border: '2px solid var(--color-primary)', background: 'var(--color-surface)', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '2rem', textAlign: 'center' }}>
                            ...
                            {resultados.admisible !== null ? (
                                <div style={{
                                    ...
                                    {resultados.admisible ? <CheckCircle2 size={32} /> : <AlertTriangle size={32} />}
                        ...
                        <div style={{ background: 'var(--color-background)', padding: '1rem', borderTop: '1px solid var(--color-border)', display: 'flex', justifyContent: 'flex-end' }}>
                            <button
                                onClick={handleSave}
                                disabled={resultados.tgbh === null}
                                className="btn-primary"
                                style={{ margin: 0, padding: '0.8rem 1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', opacity: resultados.tgbh === null ? 0.5 : 1 }}
                            >
