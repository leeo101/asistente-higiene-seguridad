                                ...
                                {capa.actions.map((action, idx) => (
                                    <div key={action.id} style={{ padding: '0.85rem 1.25rem', background: action.status === 'completed' ? 'rgba(16, 185, 129, 0.04)' : 'var(--color-background)', border: `1px solid ${action.status === 'completed' ? '#10b981' : 'var(--glass-border-subtle)'}`, borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-sm)' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                            <div style={{ width: '28px', height: '28px', background: action.status === 'completed' ? '#10b981' : 'rgba(var(--color-primary-rgb), 0.1)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyCenter: 'center', color: action.status === 'completed' ? '#fff' : 'var(--color-primary)', fontSize: '0.8rem', fontWeight: 900, flexShrink: 0, justifyContent: 'center' }}>{idx + 1}</div>
                                            <div style={{ flex: 1, minWidth: 0 }}>
