    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ fontSize: '2.5rem', margin: '0 0 0.8rem' }}>🗑️</div>
...
export default function LightingHistory(): React.ReactElement | null {
    ...
    const confirmDelete = () => {
        ...
        syncCollection('lighting_history', updated);
        ...
        <div className="container" style={{ maxWidth: '800px', paddingBottom: '5rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', minWidth: '200px' }}>
                    ...
                    <div>
                        <h1 style={{ margin: 0, fontSize: 'clamp(1.1rem, 4vw, 1.4rem)', fontWeight: 800, lineHeight: 1.2 }}>Iluminación</h1>
                ...
                >
                    <Plus size={18} /> <span className="hidden sm:inline">Nuevo Estudio</span>
                </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {filteredHistory.length > 0 ? (
                        ...
                        <div key={item.id} className="card" style={{ padding: '1.2rem', transition: 'transform 0.2s' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: '1rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', minWidth: 0, flex: 1 }}>
                                    <div style={{ width: '45px', height: '45px', background: 'rgba(234, 179, 8, 0.1)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#eab308', flexShrink: 0 }}>
                                        <Lightbulb size={21} />
                                    </div>
                                    <div style={{ minWidth: 0, flex: 1 }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.8rem', color: 'var(--color-text-muted)', marginTop: '0.2rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                            <Calendar size={14} style={{ flexShrink: 0 }} /> <span style={{ flexShrink: 0 }}>{new Date(item.date || item.createdAt).toLocaleDateString('es-AR')}</span> - <Building2 size={14} style={{ flexShrink: 0 }} /> <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.sector || 'Sin sector'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.8rem', marginTop: '1rem', borderTop: '1px solid var(--color-border)', paddingTop: '1rem' }}>
                                <button
                                    onClick={() => navigate('/lighting', { state: { editData: item } })}
                    ...
                    <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--color-text-muted)' }}>
                        ...
                        <p style={{ marginBottom: '1.5rem' }}>No se encontraron estudios de iluminación.</p>
                        <button onClick={() => navigate('/lighting')} className="btn-primary" style={{ margin: '0 auto' }}>Realizar nuevo Estudio</button>
                    </div>
                )}
            </div>
            {qrTarget && (
