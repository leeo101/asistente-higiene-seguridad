...
import {
    ArrowLeft, Search, Package, AlertTriangle, CheckCircle2, Clock, Thermometer, Droplets
} from 'lucide-react';
...
export default function ConfinedSpaceHistory() {
    ...
    useEffect(() => {
        const stored = JSON.parse(localStorage.getItem('confined_space_permits') || '[]');
        setPermits(stored);
    }, []);

    const filteredPermits = permits.filter(permit => {
        const matchesSearch = permit.location?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                            permit.supervisor?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    return (
        <div className="container" style={{ paddingBottom: '6rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                ...
                <button onClick={() => navigate('/confined-space')} className="btn-primary" style={{ margin: 0, padding: '0.75rem 1.25rem' }}>
                    Nuevo Permiso
                </button>
            </div>
            ...
            {filteredPermits.length === 0 ? (
                <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
                    ...
                    <button onClick={() => navigate('/confined-space')} className="btn-primary" style={{ marginTop: '1rem' }}>Crear Primer Permiso</button>
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {filteredPermits.map(permit => (<PermitCard key={permit.id} permit={permit} onClick={() => navigate(`/confined-space/${permit.id}`)} />))}
                </div>
            )}
        </div>
    );
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>{permit.supervisor || 'Sin supervisor'} • {permit.date ? new Date(permit.date).toLocaleDateString() : 'Sin fecha'}</p>
                    </div>
                </div>
