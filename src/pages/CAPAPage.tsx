    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--color-background)', paddingBottom: isMobile ? '80px' : '2rem' }}>
            ...
            <div style={{ padding: isMobile ? '0 1rem' : '0 1.5rem', maxWidth: '1400px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {filtered.length === 0 ? <EmptyState onAdd={() => navigate('/capa/new')} isMobile={isMobile} /> : filtered.map(c => (<CapaCard key={c.id} capa={c} statusConfig={(STATUS as any)[c.status] || STATUS.open} priorityConfig={(PRIORITY as any)[c.priority] || PRIORITY.medium} onStart={() => updateStatus(c.id, 'in_progress')} onComplete={() => updateStatus(c.id, 'completed')} onView={() => setSelected(c)} onDelete={() => del(c.id)} isMobile={isMobile} />))}
            </div>
            </div>
