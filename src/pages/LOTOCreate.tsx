            ...
            <div className="card" style={{ padding: '2.5rem', paddingTop: '2rem' }}>
                ...
                <div className="form-section">
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                        <div style={{ gridColumn: '1 / -1' }}><label style={labelStyle}>Nombre del Equipo *</label><input className="input-professional" type="text" value={procedure.equipmentName} onChange={(e) => setProcedure({ ...procedure, equipmentName: e.target.value })} style={inputStyle} /></div>
                        <div><label style={labelStyle}>Área/Ubicación</label><input className="input-professional" type="text" value={procedure.area} onChange={(e) => setProcedure({ ...procedure, area: e.target.value })} style={inputStyle} /></div>
                        <div><label style={labelStyle}>Responsable</label><input className="input-professional" type="text" value={procedure.responsible} onChange={(e) => setProcedure({ ...procedure, responsible: e.target.value })} style={inputStyle} /></div>
                        <div><label style={labelStyle}>Fecha</label><input className="input-professional" type="date" value={procedure.date} onChange={(e) => setProcedure({ ...procedure, date: e.target.value })} style={inputStyle} /></div>
                        <div><label style={labelStyle}>Puntos de Bloqueo</label><input className="input-professional" type="text" value={procedure.lockoutPoints} onChange={(e) => setProcedure({ ...procedure, lockoutPoints: e.target.value })} style={inputStyle} /></div>
                    </div>
                </div>
                ...
                <div className="form-section">
                    <div className="form-section-header">
                        ...
                        <span className="section-title">Tipos de Energía a Bloquear</span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.75rem' }}>
                        {ENERGY_TYPES.map(type => (
                            <button key={type.id} onClick={() => toggleEnergyType(type.id)} style={{ padding: '1rem', background: procedure.energyTypes?.includes(type.id) ? '#dc262620' : 'var(--color-background)', border: `2px solid ${procedure.energyTypes?.includes(type.id) ? '#dc2626' : 'var(--color-border)'}`, borderRadius: 'var(--radius-lg)', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', transition: 'all var(--transition-fast)' }}>
                ...
                <div className="form-section">
                    <div className="form-section-header">
                        ...
                        <span className="section-title">Proceso de Bloqueo</span>
                    </div>
                    <div><label style={labelStyle}>Pasos del Procedimiento</label><textarea className="input-professional" value={procedure.steps} onChange={(e) => setProcedure({ ...procedure, steps: e.target.value })} style={{ ...inputStyle, minHeight: '100px' }} placeholder="Describí los pasos para el bloqueo..." /></div>
                    <div><label style={labelStyle}>Observaciones</label><textarea className="input-professional" value={procedure.observations} onChange={(e) => setProcedure({ ...procedure, observations: e.target.value })} style={{ ...inputStyle, minHeight: '60px' }} /></div>
                </div>
