...
export default function RARForm(): React.ReactElement | null {
  ...
  const handleLoadTypicalCrew = () => {
    ...
    toast.success('Cuadrilla tipo con 5 perfiles cargada');
  };
  ...
  const handleSave = () => {
    ...
    if (isEdit) {
      ...
      toast.success('Nómina RAR actualizada');
    } else {
  ...
  const stats = calculateRARStats(survey.trabajadores || []);
  ...
  return (
    <ModuleFormLayout>
      <ModuleFormToolbar
        ...
        subtitle="Resolución S.R.T. N° 37/10 · Relevamiento de Agentes de Riesgo"
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        {/* Banner Informativo */}
        <div className="bg-gradient-to-r from-emerald-900/90 to-teal-900/90 text-white rounded-xl p-4 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                Nómina de Expuestos Res. SRT 37/10
              ...
              <p className="text-xs text-emerald-100 max-w-2xl">
                Declare a la ART los agentes de riesgo (Dec. 658/96) a los que se exponen los operarios para coordinar los exámenes médicos periódicos (ESOP) anuales.
              </p>
            </div>
          </div>
          <button
        ...
        {/* Resumen en Vivo */}
        <div className="p-4 rounded-xl border border-emerald-300 dark:border-emerald-800/60 bg-emerald-50/60 dark:bg-emerald-950/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-emerald-600 text-white flex flex-col items-center justify-center font-black">
              <span className="text-lg leading-none">{stats.porcentajeExpuestos}%</span>
              <span className="text-[8px] uppercase tracking-wide opacity-90 mt-0.5">Expuestos</span>
            </div>
            <div>
              <div className="text-sm font-black uppercase tracking-wide text-slate-900 dark:text-slate-100">
                {stats.trabajadoresExpuestos} trabajadores expuestos de {stats.totalTrabajadores} relevados
              </div>
              <div className="text-xs text-slate-600 dark:text-slate-300">
                {stats.trabajadoresNoExpuestos} operarios sin agentes de riesgo declarados (administrativos/no expuestos).
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {stats.topAgentes.map(ag => (
              <span key={ag.codigo} className="px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/40 text-emerald-900 dark:text-emerald-200 text-[10px] font-bold font-mono">
                {ag.codigo}: {ag.cantidad}
              </span>
            ))}
          </div>
        </div>
        ...
        <ModuleFormSection title="1. Datos de la Empresa y ART" icon={<Building2 size={20} />}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Ruta 8 Km 60, Pilar"
                className="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100"
              />
            </div>
            <div>
        ...
        <ModuleFormSection title={`2. Nómina de Trabajadores (${survey.trabajadores.length})`} icon={<UserPlus size={20} />}>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
                  ...
                  <div className="sm:col-span-3">
                    <label className="block text-[10px] font-bold text-slate-500 uppercase">CUIL / DNI *</label>
                    <input
                      type="text"
                      value={w.cuil}
                      onChange={e => updateWorker(w.id, 'cuil', e.target.value)}
                      placeholder="20-12345678-9"
                      className="w-full px-2 py-1 text-xs font-mono font-bold rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
                    />
                  </div>
