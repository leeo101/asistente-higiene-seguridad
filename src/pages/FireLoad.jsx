...
export default function FireLoad() {
    ...
    return (
        <div className="container" style={{ maxWidth: '900px', paddingBottom: '5rem' }}>
            <style>
                {`
                    @media print {
                        ...
                        nav, .sidebar, button, .no-print, .sticky-container, .plus-button, .header-actions {
                    ...
                    .print-header, .print-only, .print-text-box { display: none; }
                `}
            </style>
            ...
            <div className="print-header">
                ...
                <div style={{ textAlign: 'left', marginTop: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', fontSize: '0.9rem' }}>
                    ...
                    <div style={{ gridColumn: '1 / span 2' }}>
                        <strong>Actividad:</strong> {formData.actividadResumen || 'No especificada'}
                    </div>
                </div>
            </div>
            ...
            <div className="main-layout" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 300px', gap: '1.5rem', alignItems: 'start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div className="card">
                        ...
                        <div style={{ marginBottom: '1rem' }}>
                            ...
                            <textarea
                                value={typeof formData.descripcionActividad === 'string' ? formData.descripcionActividad : ''}
                            ...
                            <div className="print-text-box">
                                {typeof formData.descripcionActividad === 'string' ? (formData.descripcionActividad || 'Sin descripción detallada.') : 'Sin descripción detallada.'}
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                            ...
                            {(formData.materiales || []).map((m, idx) => (
                                <React.Fragment key={idx}>
                                    ...
                                    <div className="print-only material-row">
                ...
                <div style={{ position: 'sticky', top: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }} className="results-grid">
                    ...
                    <button className="no-print btn-primary" onClick={handleSave} style={{ width: '100%', padding: '1rem', display: 'flex', justifyContent: 'center', gap: '0.6rem' }}>
                        <Save size={20} /> Guardar Estudio
                    </button>

                    <button className="no-print" onClick={handlePrint} style={{ width: '100%', padding: '1rem', border: '1px solid var(--color-border)', borderRadius: '10px', cursor: 'pointer', fontWeight: 600, background: 'var(--color-surface)', color: 'var(--color-text)', display: 'flex', justifyContent: 'center', gap: '0.6rem' }}>
                        <Printer size={20} /> Generar Informe
                    </button>
                </div>
            </div>

            <div className="card" style={{ marginTop: '2.5rem', borderTop: '2px solid var(--color-primary)', display: 'block' }}>
                <h3 style={{ marginTop: 0, marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
                    <ShieldCheck size={22} color="var(--color-primary)" /> Responsable del Estudio
                </h3>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '1.5rem', background: 'rgba(59, 130, 246, 0.05)', borderRadius: '12px' }}>
                    {professional.signature ? (
                        <img src={professional.signature} alt="Firma Profesional" style={{ height: '90px', maxWidth: '200px', objectFit: 'contain', border: '1px dashed #ccc', padding: '5px' }} />
                    ) : (
                        <div style={{ height: '90px', width: '200px', border: '1px dashed #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', color: '#999', borderRadius: '8px' }}>Firma no registrada</div>
                    )}
                    <div>
                        <p style={{ margin: 0, fontWeight: 800, fontSize: '1.1rem', color: 'var(--color-text)' }}>{professional.name}</p>
                        <p style={{ margin: '0.2rem 0', fontSize: '0.9rem', color: 'var(--color-text-muted)', fontWeight: 600 }}>Profesional de Higiene y Seguridad</p>
                        <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--color-primary)', fontWeight: 700 }}>Matrícula: {professional.license}</p>
                    </div>
                </div>
            </div>
        </div>
