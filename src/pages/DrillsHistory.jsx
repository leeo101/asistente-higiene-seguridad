            ...
            {deleteTarget && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
                    <div className="card" style={{ maxWidth: '320px', textAlign: 'center', padding: '2rem' }}>
                        ...
                        <h3>¿Eliminar acta?</h3>
                        <p style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)' }}>Esta acción borrará definitivamente el registro de {history.find(h => h.id === deleteTarget)?.empresa}.</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {filteredHistory.map((report) => (
                    <React.Fragment key={report.id}>
                        <div className="card" style={{ padding: '1.25rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '1rem', borderLeft: `6px solid #f97316` }}
                        ...
                        <div style={{ marginTop: '0.5rem', marginBottom: '1.5rem', padding: '0 1rem', display: 'flex', justifyContent: 'flex-end', gap: '0.8rem' }}>
                            ...
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    navigate('/drills', { state: { editData: report } });
                                }}
