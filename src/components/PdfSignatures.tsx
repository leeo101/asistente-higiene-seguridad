    ...
    return (
        <div style={{ width: '100%', pageBreakInside: 'avoid', breakInside: 'avoid', display: 'inline-block' }}>
            ...
            {boxes.map((box, idx) => {
                ...
                return (
                    <div key={idx} style={{ flex: '0 1 32%', border: `1px solid ${borderCol}`, background: bgCol, borderRadius: '6px', padding: '0.8rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <div style={{ height: '60px', width: '100%', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: '0.4rem', borderBottom: `1px solid ${lineCol}`, paddingBottom: '0.25rem', marginBottom: '0.5rem' }}>
                            {box.signatureUrl && (
                                <img src={box.signatureUrl} alt="Firma" style={{ maxHeight: '50px', maxWidth: box.stampUrl ? '48%' : '95%', objectFit: 'contain' }} />
                            )}
                            {box.stampUrl && (
                                <img src={box.stampUrl} alt="Sello" style={{ maxHeight: '50px', maxWidth: box.signatureUrl ? '48%' : '95%', objectFit: 'contain' }} />
                            )}
                            {!box.signatureUrl && !box.stampUrl && (
                                <span style={{ fontSize: '0.6rem', color: lineCol }}>{isPro ? 'Sello y Firma Digital' : 'Firma original'}</span>
                            )}
                        </div>
                        <p style={{ margin: 0, fontWeight: 700, fontSize: '0.7rem', color: textCol, textTransform: 'uppercase', textAlign: 'center' }}>{box.title}</p>
                        <p style={{ margin: '2px 0 0 0', fontSize: '0.6rem', color: subTextCol, fontWeight: isPro ? 600 : 400, textAlign: 'center' }}>{box.subtitle}</p>
                        {box.license && (
                            <p style={{ margin: '2px 0 0 0', fontSize: '0.55rem', color: '#16a34a', textAlign: 'center' }}>Mat.: {box.license}</p>
                        )}
                        {box.customContent}
                    </div>
                );
